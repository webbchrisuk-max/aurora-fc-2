(function(w){
  ...
  function renderTargets(state){
    ...
    const brokers=[...new Set(targets.map(t=>A().transferEngine?.brokerRouteLabel?.(state,t)||accountLabel(t.preferredAccount)).filter(x=>x!=='Broker unresolved'))];
    set('targetBoardCoverage',brokers.length?brokers.join(' + '):'Broker unresolved');
    ...
    const broker=t=>String(A().transferEngine?.effectiveBroker?.(state,t)||accountCode(t.preferredAccount)).toUpperCase();
    const brokerDisplay=t=>A().transferEngine?.brokerRouteLabel?.(state,t)||accountLabel(broker(t));
