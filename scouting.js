(function(w){
  ...
  function accountLabel(v){
    ...
    return c==='IG'?'IG ISA':c==='T212'?'Trading 212 ISA':'Broker unresolved';
  }
  function brokerRoute(state,target){
    return A().transferEngine?.resolveBrokerRoute?.(state,target)||{account:accountCode(target?.preferredAccount),eligible:[],supported:accountCode(target?.preferredAccount)!=='CHECK'};
  }
  function brokerLabel(state,target){
    const route=brokerRoute(state,target);
    return A().transferEngine?.brokerRouteLabel?.(route)||accountLabel(route.account);
  }
