(function(w){
  ...
  let publishing=false,backendBusy=false;
  ...
  function holdingIncome(h){const shares=num(h.shares),dps=num(h.annualDpsGbp);return shares>0&&dps>0?shares*dps:Math.max(0,num(h.annualIncomeGbp))}
  ...
  function metrics(state=A().core.read()){
    const holdings=activeHoldings(state),byAccount=new Map(),byTicker=new Map();
    ...
    return {holdings,annual,monthly,value,book,yoc:book>0?annual/book*100:0,yieldPct:value>0?annual/value*100:0,byAccount:[...byAccount.values()],players,best:players[0]||null};
  }

  function holdingForEvent(state,e){const ac=accountCode(e.account),tk=ticker(e.ticker);return activeHoldings(state).find(h=>accountCode(h.account)===ac&&ticker(h.ticker)===tk)||null}
  function eventAmount(state,e){if(String(e.status||'').toUpperCase()==='PAID'&&num(e.actualAmountGbp)>0)return num(e.actualAmountGbp);const h=holdingForEvent(state,e),dps=num(e.dividendPerShareGbp);if(h&&dps>0)return num(h.shares)*dps;return Math.max(0,num(e.expectedAmountGbp))}
  function activeCalendar(state=A().core.read()){return arr(state.income?.calendar).filter(e=>!['CANCELLED','ARCHIVED'].includes(String(e.status||'').toUpperCase()))}
  ...
  function populateHoldingSelect(state=A().core.read(),selected){const el=$('eventHolding');if(!el)return;const hs=activeHoldings(state).sort((a,b)=>ticker(a.ticker).localeCompare(ticker(b.ticker))||accountCode(a.account).localeCompare(accountCode(b.account)));el.innerHTML=hs.length?hs.map(h=>{const v=`${accountCode(h.account)}|${ticker(h.ticker)}`;return `<option value="${esc(v)}">${esc(ticker(h.ticker))} • ${esc(accountLabel(h.account))} • ${num(h.shares).toLocaleString('en-GB',{maximumFractionDigits:6})} shares</option>`}).join(''):'<option value="">No active Squad holdings</option>';if(selected&&hs.some(h=>`${accountCode(h.account)}|${ticker(h.ticker)}`===selected))el.value=selected}
  function selectedHolding(state=A().core.read()){const [ac,tk]=String($('eventHolding')?.value||'').split('|');return activeHoldings(state).find(h=>accountCode(h.account)===ac&&ticker(h.ticker)===tk)||null}
  function eventPreview(){const h=selectedHolding(),dps=Math.max(0,num($('eventDps')?.value)),auto=h&&dps>0?num(h.shares)*dps:Math.max(0,num($('eventExpected')?.value));if(h&&dps>0)setValue('eventExpected',auto.toFixed(2));set('eventPreview',h?`${ticker(h.ticker)} • ${accountLabel(h.account)} • ${num(h.shares).toLocaleString('en-GB',{maximumFractionDigits:6})} shares • expected ${money(auto)}`:'Choose an active Squad holding.');return {h,amount:auto}}
  function clearEvent(){setValue('eventId','');['eventExDate','eventPayDate','eventDps','eventExpected','eventActual','eventNotes'].forEach(id=>setValue(id,''));setValue('eventStatus','FORECAST');populateHoldingSelect();set('editorMode','NEW EVENT');eventPreview()}
  function editEvent(id){const s=A().core.read(),e=arr(s.income?.calendar).find(x=>x.id===id);if(!e)return;setValue('eventId',e.id);populateHoldingSelect(s,`${accountCode(e.account)}|${ticker(e.ticker)}`);setValue('eventHolding',`${accountCode(e.account)}|${ticker(e.ticker)}`);setValue('eventExDate',e.exDate);setValue('eventPayDate',e.payDate);setValue('eventDps',e.dividendPerShareGbp||'');setValue('eventExpected',e.expectedAmountGbp||'');setValue('eventActual',e.actualAmountGbp||'');setValue('eventStatus',e.status||'FORECAST');setValue('eventNotes',e.notes||'');set('editorMode',`EDIT ${ticker(e.ticker)}`);eventPreview();document.querySelector('article:has(#eventHolding)')?.scrollIntoView({behavior:'smooth',block:'start'})}
  ...
  function renderPlayers(m){const host=$('playerList');set('playerCount',`${m.players.length} player${m.players.length===1?'':'s'}`);if(!host)return;if(!m.players.length){host.innerHTML='<div class="empty-state compact"><strong>No income-producing holdings</strong><p>Squad needs active holdings with annual DPS or annual income evidence.</p></div>';return}host.innerHTML=m.players.map((p,i)=>`<article class="player-row ${i===0?'top':''}"><div class="row-copy"><strong>#${i+1} • ${esc(p.ticker)} — ${esc(p.name)}</strong><span>${p.accounts.map(accountLabel).join(' + ')} • ${p.shares.toLocaleString('en-GB',{maximumFractionDigits:6})} shares • ${p.book>0?(p.annual/p.book*100).toFixed(2):'0.00'}% yield on cost</span></div><div class="row-side"><strong>${money(p.annual)}</strong><span>${money(p.annual/12)} / month</span></div></article>`).join('')}
