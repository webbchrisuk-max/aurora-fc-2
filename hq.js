(function(w){
  ...
  function render(){
    ...
    ui.text('nextDividend',nd?.ticker||'—');
    ui.text('nextDividendMeta',nd?`${ui.money(nd.amount)} • ${nd.date||'date pending'} • ${nd.account||''}`:'Add or sync the dividend calendar in Income');
    const status=String(s.connection?.status||'').toUpperCase();
    ui.text('connectionState',['LIVE','CONNECTED'].includes(status)?'LIVE':'LOCAL');
    renderMission(s.mission,ui);renderAlerts(s.alerts||[],ui);ui.text('lastUpdated',new Date(s.updatedAt).toLocaleString('en-GB'));
  }
  ...
  function renderAlerts(alerts,ui){const host=document.getElementById('alerts');if(!host)return;if(!alerts.length){host.innerHTML='<div class="notice">No live alerts right now.</div>';return;}host.innerHTML=alerts.slice(0,4).map(a=>`<div class="alert-row"><i class="alert-dot"></i><div><strong>${ui.escape(a.title)}</strong><span>${ui.escape(a.note||'')}</span></div><time>${ui.escape(a.when||'')}</time></div>`).join('');}
