/* Aurora City FC — Nexus V2 Canonical Controller v1
...
const BUILD='20260819-nexus-clean-1';
const positions=[
 {left:50,top:88},
...
function permittedTargets(s){return arr(s?.scouting?.targets).filter(t=>!t?.restricted&&!['BLOCK','BLOCKED','RESTRICTED','REJECTED'].includes(String(t?.status||t?.recommendation||'').toUpperCase()))}
...
function renderIntel(s,g){
 ...
 const fields=[
  ...
  ['Best permitted',op?`${ticker(op.ticker||op.name)} ${targetScore(op,s).toFixed(0)}/100`:'—',op?'Permitted Scouting route':'No permitted target'],
...
function installPitchGeometry(){const pitch=document.querySelector('.pitch-panel .pitch');if(!pitch)return;['n2-halfway-line','n2-centre-circle','n2-centre-spot','n2-pitch-box top','n2-pitch-box bottom'].forEach(()=>{});if(!pitch.querySelector('.n2-halfway-line'))pitch.insertAdjacentHTML('afterbegin','<div class="n2-halfway-line"></div><div class="n2-centre-circle"></div><div class="n2-centre-spot"></div><div class="n2-pitch-box top"></div><div class="n2-pitch-box bottom"></div>')}
...
function renderLeaders(s,g){
 ...
 const cards=[
  ...
  ['Best Opportunity',op,op?`${targetScore(op,s).toFixed(0)}/100`:'—','Permitted Scouting route'],
...
function latestReport(s){return s?.matchday?.latest||s?.matchdayReport?.latest||s?.matchdayReport||s?.matchReport?.latest||null}
function reportDate(r){return parseDate(r?.report_date||r?.reportDate||r?.generated_at||r?.generatedAt||r?.date)}
function renderMatchday(s){const r=latestReport(s),today=new Date(),d=reportDate(r),same=d&&d.toDateString()===today.toDateString(),status=document.getElementById('matchdayStatus'),result=document.getElementById('matchdayResult'),summary=document.getElementById('matchdaySummary');if(!status||!result)return;if(same){status.textContent=r?.recovered?'RECOVERED':'PUBLISHED';const p=raw(r?.portfolio_change_pct??r?.portfolioChangePct),g=raw(r?.portfolio_change_gbp??r?.portfolioChangeGbp);result.textContent=p!==null?`${p>0?'+':''}${p.toFixed(2)}%${g!==null?` • ${signedMoney(g)}`:''}`:'Full-time report';result.className=p>0?'positive':p<0?'negative':'';if(summary)summary.textContent=String(r?.summary||r?.manager_report||r?.managerReport||'Today’s published portfolio report.')}else{status.textContent=today.getHours()<17?'DUE 5PM':'AWAITING REPORT';result.textContent=today.getHours()<17?'Market in progress':'Checking full-time feed';result.className='warning';if(summary)summary.textContent=today.getHours()<17?'The published full-time report will appear after the market session.':'Aurora is checking the canonical MatchdayReport feed.'}}
...
function render(){const s=state();if(!s)return;const g=aggregate(s);renderHero(s,g);renderIntel(s,g);renderPitch(g);renderLeaders(s,g);renderMomentum(g);renderBrokers(s);renderRoute(s);renderRunway(s);renderLeagues(g);renderSectors(g);renderIncome(s,g);renderMission(s);renderHealth(s,g);renderMatchday(s);if(currentDrawerTicker)openDrawer(currentDrawerTicker);document.body.classList.remove('loading');document.documentElement.dataset.nexusController='canonical'}
...
function init(){ensureDrawer();render();hydrateMaster();[250,900,1800].forEach(ms=>setTimeout(schedule,ms));document.addEventListener('click',e=>{const lens=e.target.closest('[data-lens]');if(lens){document.querySelectorAll('.lens').forEach(x=>x.classList.toggle('active',x===lens));schedule();return}const player=e.target.closest('[data-n2-player]');if(player){openDrawer(player.dataset.n2Player);return}if(e.target.closest('#n2DrawerClose')||e.target===document.getElementById('n2PlayerDrawerBackdrop'))closeDrawer()});document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDrawer()});w.addEventListener('aurora2:state',schedule);w.addEventListener('aurora:market-live',schedule);w.addEventListener('aurora2:nexus-hydrated',()=>{schedule();hydrateMaster(true)});document.addEventListener('visibilitychange',()=>{if(!document.hidden){schedule();hydrateMaster()}});setInterval(()=>{if(!document.hidden)schedule()},15000);setInterval(()=>{if(!document.hidden)hydrateMaster()},20*60*1000)}
w.AuroraNexusCanonical={build:BUILD,render:schedule,refresh:async()=>{try{await w.AuroraClubCommand?.refreshMarket?.()}catch(_){}await hydrateMaster(true);schedule()},snapshot:()=>{const s=state();return s?aggregate(s):[]}};
