...
(function(){
  ...
  function inspect(el){
    if(!(el instanceof HTMLElement))return;const text=el.textContent.trim(),old=previous.get(el);
    if(old===undefined){previous.set(el,text);return}if(text===old)return;previous.set(el,text);if(!armed)return;
  ...
  function loadScriptForPage(page,selector,src,datasetKey){
    const file=String(location.pathname||'').split('/').pop().toLowerCase();if(file!==page||document.querySelector(selector))return;
    const script=document.createElement('script');script.src=src;script.async=false;script.dataset[datasetKey]='1';document.head.appendChild(script);
  }
  ...
  function loadScoutingCleanBoard(){loadScriptForPage('scouting.html','script[data-aurora-scouting-clean-board]','scouting-clean-board.js?v=20260818-clean-board-1','auroraScoutingCleanBoard')}
  ...
  function init(){
    remember();animateRows(document);scoutingHooks();
    loadScoutingIntelligence3();loadFormerReentryRefresh();loadScoutingSquadOpportunities();loadScoutingCleanBoard();loadBackgroundSignalScouting();
    loadFinanceBillActionsFix();loadFinanceHouseDashboardUpgrade();loadFinanceHouseRoomGroups();loadFinanceHousePriorityLayout();
    observer.observe(document.body,{subtree:true,childList:true,characterData:true});requestAnimationFrame(()=>requestAnimationFrame(()=>{armed=true;document.documentElement.classList.add('aurora-motion-ready')}));
  }
